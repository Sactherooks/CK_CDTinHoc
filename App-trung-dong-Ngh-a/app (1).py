...
# ---- Base directory (thư mục chứa app.py) ----
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

def find_asset(suffix):
    """Tìm file trong cùng thư mục với app.py dựa theo phần cuối tên file."""
...
class Player:
    def __init__(self, screen_rect):
        ...
        self.image_idle = try_load_image(find_asset("mc_normal.png"), (self.width, self.height))
        self.frames = [
            try_load_image(find_asset("131643.png"), (self.width, self.height)),
            try_load_image(find_asset("131649.png"), (self.width, self.height)),
        ]
        self.image_parry = try_load_image(find_asset("134240.png"), (self.width, self.height))
...
class Projectile:
    def __init__(self, screen_rect, speed):
        ...
        self.image = try_load_image(find_asset("aura.png"), (self.width, self.height))
...
class Item:
    def __init__(self, screen_rect):
        ...
        self.image = try_load_image(find_asset("baogay.png"), (self.width, self.height))
        if self.image is None:
            self.image = pygame.Surface((self.width, self.height), pygame.SRCALPHA)
...
def main():
    ...
    # Background
    bg_image = try_load_image(find_asset("244.png"), (SCREEN_WIDTH, SCREEN_HEIGHT))
    if bg_image is None:
    ...
    invincible_timer = 0
    anim_frames = [
        try_load_image(find_asset("131904.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
        try_load_image(find_asset("131946.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
        try_load_image(find_asset("131950.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
        try_load_image(find_asset("131954.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
        try_load_image(find_asset("131958.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
        try_load_image(find_asset("132001.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
        try_load_image(find_asset("132004.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
        try_load_image(find_asset("132007.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
        try_load_image(find_asset("132010.png"), (SCREEN_WIDTH, SCREEN_HEIGHT)),
    ]
    anim_index = 0
    anim_timer = 0
    playing_anim = False
